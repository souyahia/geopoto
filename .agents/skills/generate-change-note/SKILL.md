...
**Keep every language note short.** Google Play rejects release notes longer than 500 characters per language, and some languages (French, Spanish, Portuguese) run noticeably longer than English. To stay safe on every language, keep each note under about 450 characters, including the header, bullets and line breaks. Prefer a handful of short bullet points, merge minor related changes into a single bullet, and drop the least important items rather than going over. After writing the notes, verify that each language stays within the limit.
...
- Each locale block must stay under the Google Play limit of 500 characters (aim for under about 450). If any language is too long, shorten the canonical note and update every language so the iOS and Android text stay identical.
