...
interface QuizzFlagAnswerProps {
...
const FLAG_ANSWER_LIST_HEIGHT = 280;
...
function getFlagAnswerGridMetrics({
  ...
  const flagWidth = Math.max(
    1,
    Math.min(
      FLAG_ANSWER_ICON_MAX_WIDTH,
      itemWidth - FLAG_ANSWER_FRAME_PADDING * 2,
    ),
  );
  const itemHeight =
    Math.round(flagWidth / FLAG_ANSWER_ICON_ASPECT_RATIO) +
...
function getFlagAnswerCountries({
  ...
  const countries = COUNTRIES.filter((country) =>
    country.regions.includes(answerRegion),
...
export function QuizzFlagAnswer({
  ...
  const flagCountries = useMemo(
    () => getFlagAnswerCountries({ answerRegion, geoLang }),
    [answerRegion, geoLang],
  );
  ...
  return (
    <View className="gap-2">
      ...
      <View className="flex-row items-center gap-3">
        <FlagIcon code={countryCode} width={48} />
...
function FlagAnswerGridItemComponent({
  ...
  const containerStyle = useMemo<ViewStyle>(
    () => ({
      height: itemHeight,
      marginBottom: FLAG_ANSWER_GRID_ROW_GAP,
      width: itemWidth,
    }),
  ...
  const content = (
    <View
      className={[
        "items-center justify-center rounded-lg border bg-field p-1",
...
function getFlagAnswerGridItemClassName({
  ...
  if (isSelected) {
    return "border-2 border-danger bg-danger/10";
  }
