...
const COUNTRY_FLAG_IMAGE_CONTEXT = require.context(
  "../generated/flags-png",
  false,
  /\.png$/,
);
const COUNTRY_FLAG_IMAGE_PATHS = new Set(COUNTRY_FLAG_IMAGE_CONTEXT.keys());

function toCountryFlagImagePath(code: string): string {
  return `./${code.toLowerCase()}.png`;
}

export function getCountryFlagImage(code: string): CountryFlagImage | null {
  const flagImagePath = toCountryFlagImagePath(code);

  if (!COUNTRY_FLAG_IMAGE_PATHS.has(flagImagePath)) {
    return null;
  }

  return COUNTRY_FLAG_IMAGE_CONTEXT<CountryFlagImage>(flagImagePath);
}
